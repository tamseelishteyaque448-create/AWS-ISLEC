...
export function ProjectMembershipControl({ projectId, membershipStatus, joinRequestStatus, canRequest }: { projectId: string; membershipStatus: string | null; joinRequestStatus?: string | null; canRequest: boolean }) {
  ...
  return <form action={formAction}><input type="hidden" name="project_id" value={projectId} />{membershipStatus !== "active" ? <><input name="contribution" maxLength={120} placeholder="How can you contribute?" /><input name="message" maxLength={1000} placeholder="Optional message" /></> : null}<button className="button" type="submit" disabled={pending}>{pending ? <LoaderCircle size={15} aria-hidden="true" /> : membershipStatus === "active" ? <Send size={15} aria-hidden="true" /> : <Check size={15} aria-hidden="true" />}{membershipStatus === "active" ? "Submit work" : "Request access"}</button>{state.message ? <p className={`event-registration-message ${state.status}`}>{state.message}</p> : null}</form>;
}
