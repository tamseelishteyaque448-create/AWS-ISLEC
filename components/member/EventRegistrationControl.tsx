...
export function EventRegistrationControl({ eventId, registrationStatus, registrationOpen, cancellationOpen }: { eventId: string; registrationStatus: "registered" | "attended" | "cancelled" | null; registrationOpen: boolean; cancellationOpen: boolean }) {
  ...
  if (registrationStatus === "cancelled") return <span className="tag">Registration cancelled</span>;
  if (registrationStatus === "registered") return <form action={formAction}><input type="hidden" name="event_id" value={eventId} /><button className="button button-secondary" type="submit" disabled={pending || !cancellationOpen}>{pending ? <LoaderCircle size={15} aria-hidden="true" /> : <X size={15} aria-hidden="true" />}{cancellationOpen ? "Cancel registration" : "Registered"}</button>{state.message ? <p className={`event-registration-message ${state.status}`} role={state.status === "error" ? "alert" : "status"}>{state.message}</p> : null}</form>;
