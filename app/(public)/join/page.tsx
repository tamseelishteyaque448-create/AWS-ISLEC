...
export default function JoinPage() {
  ...
  return <>
    ...
    <section className="join-section join-benefits" aria-labelledby="benefits-title"><div className="join-section-heading"><div><div className="eyebrow">What members get</div><h2 id="benefits-title">A little more direction. A lot more momentum.</h2></div><p>Make progress at your own pace, with practical places to focus and people who understand why the work matters.</p></div><div className="join-benefit-grid"><article><span className="join-benefit-icon"><BookOpen size={21} aria-hidden="true" /></span><strong>{challenges.length} practical paths</strong><p>Build confidence through focused challenges that lead somewhere concrete.</p><Link href="/learn">Explore learning <ArrowRight size={14} aria-hidden="true" /></Link></article><article><span className="join-benefit-icon"><Rocket size={21} aria-hidden="true" /></span><strong>{projects.length} projects in motion</strong><p>Find work to follow, questions to borrow, and a studio for your own experiments.</p><Link href="/projects">See projects <ArrowRight size={14} aria-hidden="true" /></Link></article><article><span className="join-benefit-icon"><UsersRound size={21} aria-hidden="true" /></span><strong>{events.filter((event) => getEventStatus(event) === "Upcoming").length} ways to gather</strong><p>Join study halls, workshops, and demo nights built for showing up as you are.</p><Link href="/events">View events <ArrowRight size={14} aria-hidden="true" /></Link></article></div></section>
