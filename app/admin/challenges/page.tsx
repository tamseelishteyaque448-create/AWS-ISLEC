...
import { LearningOutcomes } from "@/components/admin/LearningOutcomes";
import { PageIntro } from "@/components/cards/PageIntro";
import { requireAdmin } from "@/lib/auth/admin";
import { getAdminChallenges } from "@/lib/services/admin-challenges";
import { getAdminLearningOutcomes } from "@/lib/services/admin-learning-outcomes";

export default async function AdminChallengesPage() {
  await requireAdmin();
  const [data, outcomes] = await Promise.all([
    getAdminChallenges().catch(() => null),
    getAdminLearningOutcomes().catch(() => null),
  ]);
  return <>
    ...
    {outcomes ? <LearningOutcomes outcomes={outcomes} /> : <section className="admin-member-error"><h2>Learning outcomes are unavailable.</h2><p>Please refresh the page and try again.</p></section>}
  </>;
}
