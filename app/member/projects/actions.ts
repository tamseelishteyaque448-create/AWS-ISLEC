...
export async function requestProjectJoin(_: ProjectMemberState, formData: FormData): Promise<ProjectMemberState> {
  ...
  if (error) return { status: "error", message: "This project is not accepting your request." }; revalidatePath("/member/projects"); revalidatePath("/projects"); return { status: "success", message: "Join request sent." };
}
