...
export async function deleteMemberProject(_: ProjectMemberState, formData: FormData): Promise<ProjectMemberState> { if (!(await getAuthenticatedClaims())?.sub) return { status: "error", message: "Please sign in first." }; const id = formData.get("project_id"); if (typeof id !== "string" || !UUID.test(id)) return { status: "error", message: "Invalid project." }; const { error } = await (await createClient()).rpc("delete_project_v1", { p_project_id: id }); if (error) return { status: "error", message: "The project could not be deleted. Confirm that you are the active owner." }; revalidatePath("/member/projects"); revalidatePath("/member/explore"); revalidatePath(`/member/projects/${id}`); revalidatePath("/admin/projects"); redirect("/member/projects"); }
