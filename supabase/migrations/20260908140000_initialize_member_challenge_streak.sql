-- A first challenge should establish momentum for members without a recorded
-- streak date. Non-zero legacy streaks remain intact until their next dated run.
...
begin
  ...
  if v_profile.streak_last_date is null then
    v_streak := case when v_profile.streak > 0 then v_profile.streak else 1 end;
